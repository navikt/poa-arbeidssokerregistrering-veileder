<!-- This file mirrors AGENTS.md and is automatically loaded by Zed's assistant. Keep both files in sync. -->

# Project Guidance

This is **poa-arbeidssokerregistrering-veileder** — a Next.js 16 (App Router) application that lets NAV counselors ("veiledere") manage a person's job-seeker status within NAV's internal tool Modia. It communicates with several backend services via server functions and OBO (On-Behalf-Of) token exchange.
...
- **No `console.log`:** Error (use `console.error`, `console.warn`, or `console.debug` instead)
